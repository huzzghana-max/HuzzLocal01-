...
if ($mysqlCheck.TcpTestSucceeded) {
    ...
    Write-Host "MySQL might not be running on port 3306" -ForegroundColor Red
...
$backendProcess = Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$serverRoot'; npm start" -PassThru
...
$frontendProcess = Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$projectRoot'; npm run dev" -PassThru
