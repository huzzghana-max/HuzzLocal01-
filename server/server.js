...
let messagesHasBookingIdCache = null
...
function normalizeServiceTitleForDisplay(rawTitle, category, vendorName) {
  const title = String(rawTitle || '').trim()
  const normalizedVendorName = String(vendorName || '').trim().toLowerCase()

  if (title) {
    const normalizedTitle = title.toLowerCase()
    if (!normalizedVendorName || normalizedTitle !== normalizedVendorName) {
      return title
    }
  }
  ...
  if (normalizedCategory) {
    return `${normalizedCategory} Service`
  }
...
async function getMessagesHasBookingId(pool) {
  if (messagesHasBookingIdCache !== null) return messagesHasBookingIdCache
  const [bookingColumn] = await pool.execute("SHOW COLUMNS FROM messages LIKE 'booking_id'")
  messagesHasBookingIdCache = bookingColumn.length > 0
  return messagesHasBookingIdCache
}
...
// Rate limiter for public event registration (20 registrations per hour per IP)
...
app.get('/api/approved-services', async (req, res) => {
  try {
    ...
    const [providerColumnsRaw] = await pool.execute('SHOW COLUMNS FROM service_providers')
    const providerColumns = new Set(providerColumnsRaw.map((column) => column.Field))
    const serviceImageExpression = serviceColumns.has('image')
      ? 's.image'
      : serviceColumns.has('image_url')
        ? 's.image_url'
        : 'NULL'
    const publicPhoneExpression = serviceColumns.has('phone')
      ? 'COALESCE(s.phone, u.phone)'
      : 'u.phone'
    const publicLocationExpression = serviceColumns.has('location')
      ? (providerColumns.has('location') ? 'COALESCE(s.location, sp.location)' : 's.location')
      : (providerColumns.has('location') ? 'sp.location' : 'NULL')
    ...
    const [services] = await pool.execute(`
      SELECT 
        ...
        s.category, 
        s.price,
        ${publicPhoneExpression} as phone,
        ${publicLocationExpression} as location
    ...
    res.json((services || []).map((service) => ({
      ...service,
      title: normalizeServiceTitleForDisplay(service.title, service.category, service.vendor_name),
    })))
  } catch (error) {
...
app.put('/api/vendor/profile', async (req, res) => {
  try {
    ...
    const { businessName, serviceType, description, hourlyRate, profileImage, portfolioImages, phone, location } = req.body
    ...
    await pool.execute(
      `UPDATE service_providers 
       SET business_name = ?, service_type = ?, description = ?, hourly_rate = ?, 
           profile_image = ?, portfolio_images = ?, location = ?
       WHERE user_id = ?`,
      [
        businessName,
        serviceType,
...
app.get('/api/vendor/services', verifyToken, async (req, res) => {
  try {
    ...
    const normalizedResults = (results || []).map((service) => ({
      ...service,
      title: normalizeServiceTitleForDisplay(service.title, service.category),
    }))
    console.log('Services found:', normalizedResults.length || 0)
...
app.post('/api/vendor/services', verifyToken, upload.single('image'), async (req, res) => {
  try {
    ...
    if (!title || !description || !category || typeof price === 'undefined' || price === null || price === '') {
...
app.get('/api/messages/:recipientId', verifyToken, async (req, res) => {
  try {
    ...
    const messagesHasBookingId = await getMessagesHasBookingId(pool)
...
app.post('/api/messages', verifyToken, async (req, res) => {
  try {
    ...
    const [result] = messagesHasBookingId
      ? await pool.execute(
      ...
      : await pool.execute(
          `INSERT INTO messages (sender_id, ${messagesRecipientColumn}, message, is_read)
           VALUES (?, ?, ?, FALSE)`,
...
app.post('/api/events/:eventId/register-public', registerLimiter, async (req, res) => {
  try {
    ...
    const [existing] = await pool.execute(
      'SELECT id, token FROM event_attendees WHERE event_id = ? AND email = ? LIMIT 1',
      [eventId, email]
    )
    if (existing.length > 0) {
      let existingQrDataUrl = null
      try {
        existingQrDataUrl = await QRCode.toDataURL(JSON.stringify({ eventId, token: existing[0].token }))
      } catch (qrErr) {
        console.error('QR regeneration failed for attendee:', qrErr.message)
      }
